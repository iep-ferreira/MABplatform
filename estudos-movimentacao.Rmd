...
carrega()
#ra_blurred
mov <- animal_mov(n = 1, ra = ra, norm_matrix = norm_matrix, time = 6, time_adapt = 0, where = "random", landscape_on = TRUE)
```

### Depois de verificar concordência entre mov e campo vetorial, 
colocar mecanismo de inércia

centriode de tempos passados é referência e, se distância aumentar, maior é 
a probabilidade de volta para a casa

```{r}
plot(mov$trajectory[[1]][,2:3], type = 'l')
```
...
# Para trajetórias
traj_df <- as.data.frame(mov$trajectory[[1]])
colnames(traj_df) <- c("time", "x", "y")
# Para campo vetorial
nrows <- nrow(ra)
ncols <- ncol(ra)
...
ggplot() +
  ...
  geom_path(data = traj_df, aes(x = x, y = y), color = "black", linewidth = 1.2, alpha = 0.4) +
