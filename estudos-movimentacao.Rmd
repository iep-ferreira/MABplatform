...
```{r}
carrega()
mov <- animal_mov(n = 5, ra = ra, time = 3, time_adapt = 1, where = "random", landscape_on = FALSE)
```

```{r}
par(mfrow = c(1,2))
plot(mov$trajectory[[1]][,2:3], type = 'l')
plot(mov$trajectory[[2]][,2:3], type = 'l', col=2)
```



### Antes da inclusão da velocidade
...
abund_model <- pcount(~ 1 ~ 1, data = umf_abund, mixture = "P")
...
```{r}
(densi.pop <- 1000/area_total) 
(dens <- m_counts / (a_q*k))
```

```{r}
carrega()
tracks.viewer(obj = mov, ra, adapt = FALSE, biomastats_std = TRUE)
```
